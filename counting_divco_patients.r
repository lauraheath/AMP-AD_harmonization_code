...
#delete specimens from 10x multiome
biospec <- subset(biospec, biospec$assay!="10x multiome")
...
#merge ampad1.0 wgs source with original biospec data
...
#how many individuals have tmt and wgs from amp1.0?
...
#identify individuals with proteomics:
tmt <- subset(wgs4, wgs4$assay=='TMT quantitation')
tmtonly <- subset(tmt, select=c(individualID, assay))
names(tmtonly)[names(tmtonly) == 'assay'] <- 'has_TMT'
tmtonly <- tmtonly[!duplicated(tmtonly),]
...
#589 have new rnaseq, protoeomics, and ANY wgs (so 81 have new rnaseq, proteomics, and OLD wgs)

#who has new proteomics & rnaseq, but NO WGS at all?
...
#13 have prot & NEW WGS, but no RNA
...
#how many people had RNAseq and new WGS only, and no proteomics?
rnaseq_wgs <- subset(tmt_rna, tmt_rna$assay=='TMT quantitation' & is.na(tmt_rna$has_RNAseq) & tmt_rna$WGSsource=="AMPAD1.0")
df <- unique(tmt_OLDwgs_NOrna$individualID)
length(df)
